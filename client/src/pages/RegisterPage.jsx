...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!formState.business_name || !formState.email || !formState.password || !formState.role) {
      setFormError("Business name, email, password, and role are required.");
      return;
    }
    ...
    if (formState.role === "supplier" && !formState.license_number) {
      setFormError("License number is required for suppliers.");
      return;
    }
  ...
  const isSupplier = formState.role === "supplier";
        ...
        <div className="relative mx-auto flex min-h-screen w-full max-w-6xl items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
          <div className="w-full max-w-xl rounded-3xl border border-slate-800 bg-slate-900/80 p-8 shadow-card">
              ...
              <p className="mt-2 text-sm text-slate-400">
                Join verified suppliers and procurement teams.
              </p>
              ...
              <form className="mt-6 grid gap-4" onSubmit={handleSubmit}>
                ...
                {isSupplier && (
                  <div>
                    <label className="text-xs font-semibold uppercase tracking-wide text-slate-400">License number</label>
                    <input
                      type="text"
                      name="license_number"
                ...
                <label className="flex items-start gap-3 rounded-2xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-xs text-slate-300">
                  <input
                    type="checkbox"
                    checked={termsAccepted}
                    onChange={(event) => setTermsAccepted(event.target.checked)}
                    className="mt-1 h-4 w-4 rounded border-slate-700 bg-slate-950 text-secondary"
                  />
                  <span>
                    I agree to the terms of service and acknowledge the onboarding policy for verified suppliers.
                  </span>
                </label>
                {(formError || error) && (
